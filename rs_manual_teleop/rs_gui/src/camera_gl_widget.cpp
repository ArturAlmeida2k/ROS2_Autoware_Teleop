...
CameraGLWidget::CameraGLWidget(QWidget* parent) : QOpenGLWidget(parent) {
    QSurfaceFormat fmt; fmt.setVersion(3, 3); fmt.setProfile(QSurfaceFormat::CoreProfile); setFormat(fmt);
}

CameraGLWidget::~CameraGLWidget() {
    makeCurrent(); delete texture_; delete shader_;
    if (gl33) { gl33->glDeleteVertexArrays(1, &vao_); gl33->glDeleteBuffers(1, &vbo_); }
    doneCurrent();
}
...
void CameraGLWidget::resizeGL(int w, int h) { gl33->glViewport(0, 0, w, h); }
...
void CameraGLWidget::paintGL() {
    gl33->glClear(GL_COLOR_BUFFER_BIT);
    {
        ...
        if (dirty_ && !pending_frame_.empty()) {
            if (!texture_->isCreated() || texture_->width() != frame_w_ || texture_->height() != frame_h_) {
                texture_->destroy(); texture_->create();
    ...
    shader_->bind(); texture_->bind();
    gl33->glBindVertexArray(vao_); gl33->glDrawArrays(GL_TRIANGLES, 0, 6);
...
void CameraGLWidget::onImageReceived(sensor_msgs::msg::CompressedImage::SharedPtr msg) {
    ...
    {
        std::lock_guard<std::mutex> lock(frame_mutex_);
        frame_w_ = frame.cols; frame_h_ = frame.rows;
...
void CameraGLWidget::setup_quad() {
    gl33->glGenVertexArrays(1, &vao_); gl33->glGenBuffers(1, &vbo_);
    gl33->glBindVertexArray(vao_); gl33->glBindBuffer(GL_ARRAY_BUFFER, vbo_);
    gl33->glBufferData(GL_ARRAY_BUFFER, sizeof(QUAD), QUAD, GL_STATIC_DRAW);
    gl33->glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)0);
    gl33->glEnableVertexAttribArray(0);
    gl33->glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)(2 * sizeof(float)));
