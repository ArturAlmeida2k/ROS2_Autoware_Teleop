...
{
    setObjectName("speed");
    setStyleSheet("QWidget#speed { background-color: rgba(0, 0, 0, 200); border-radius: 8px; }");
...
void SpeedPanel::setTurnSignal(uint8_t turn_signal)
{
    if (turn_signal == CmdEnums::TURN_LEFT) {
        ...
        lbl_turn_->setStyleSheet("color:#f39c12; background:transparent;");
    } else if (turn_signal == CmdEnums::TURN_HAZARD) {
